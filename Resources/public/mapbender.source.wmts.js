...
//                }
                ...
                if (sourceDef.configuration.origin) {
                    origin = new OpenLayers.LonLat(
                        sourceDef.configuration.origin[0],
                        sourceDef.configuration.origin[1]);
                }

                var tileSize = null;
                if (sourceDef.configuration.tileSize) {
                    tileSize = new OpenLayers.Size(
                        sourceDef.configuration.tileSize[0],
                        sourceDef.configuration.tileSize[1]);
                }

                var tileFullExtent = null;
                if (sourceDef.configuration.tileFullExtent) {
                    tileFullExtent = OpenLayers.Bounds.fromArray(
                        sourceDef.configuration.tileFullExtent);
                }
...
//                    transitionEffect: 'resize',
                ...
                mqLayerDef = {
                    type: 'wmts',
                    label: sourceDef.configuration.title,
                    url: finalUrl,
                    layer: sourceDef.configuration.layer,
                    style: sourceDef.configuration.style,
                    matrixSet: sourceDef.configuration.matrixSet,
                    matrixIds: sourceDef.configuration.matrixIds,
                    format: sourceDef.configuration.format,
                    tileOrigin: origin,
                    tileSize: tileSize,
                    tileFullExtent: tileFullExtent,
                    isBaseLayer: sourceDef.configuration.baselayer,
                    opacity: sourceDef.configuration.opacity,
                    visible: sourceDef.configuration.visible,
                    attribution: sourceDef.configuration.attribution
                };
                $.extend(mqLayerDef, Mapbender.source.wmts.defaultMqLayer);
                return mqLayerDef;
            },
            createWms: function(sourceDef) {
                var self = this;
                var rootLayer = sourceDef.configuration.children[0];
                if (sourceDef.configuration.status !== 'ok') { //deactivate corrupte or unreachable sources
                    rootLayer.options.treeOptions.selected = false;
